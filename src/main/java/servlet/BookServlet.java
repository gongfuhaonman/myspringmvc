...
//@WebServlet("/book")
//@MultipartConfig(location = "D:\\javaupload", maxFileSize = 8388608, fileSizeThreshold = 819200)
public class BookServlet extends HttpServlet {
	...
	private static final String imageDir = "D:/javaupload/";
	...
	private void listBooks(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		List<Book> books = StudentDao.getAll();
	...
	private void doEditBook(HttpServletRequest request,
		...
		Book oldBook = StudentDao.get(oldId);
		String fileName = oldBook.image;
		book.image = fileName;
		StudentDao.update(book, oldId);
		Collection<Part> parts = request.getParts();
		for (Part part : parts) {
			if (part.getName().equals("image")) {
				// 文件保存到 D:/javaupload/{book.id+System.nanoTime()}
				part.write(fileName);
				break;
			}
		}
